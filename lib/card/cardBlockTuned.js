...
module.exports=function(buffer){
    if(buffer[0]==0x76){
        var b=vuBlock(buffer,0,{})
    }else{
        var b=cardBlock(buffer,0,{})
        console.log()
    }

}
var vuBlock = function (buffer, start, obj) {
    var trep = bytestoNumber(buffer, star, 2)
    if (trep > 0x7600 && trep < 0x7606) {
        console.log(trep)
        switch (buffer[start]) {
            case "activity":
                obj[TAG[trep].block].push(blockProperties(TAG[fid].properties, 0, buffer, start, {},0))
            default:
                obj[TAG[trep].block]=blockProperties(TAG[fid].properties, 0, buffer, start, {},0)
        }
    }
    start++
    return (start > buffer.lenght - 2) ? obj : vuBlock(buffer, start+obj.size, obj)
}
...
function blockProperties(properties,i,buffer,start,obj,sizeBlock){
    ...
    if (i < properties.length) {
        ...
        if(name=="vehiclePointerNewestRecord"){
            console.log()
        }
...
function propertiesArray(buffer,start,size,end,properties,array,sizeBlock){
    ...
    switch (properties[0].name){
        ...
        case "faultType":
            ...
            break
        default:
            array=normalArray(buffer,start,size,end,properties,array,sizeBlock)
            break
    }
    return array
...
function ActivityChangeInfo(buffer,start,size,obj){
   return propertieActivityChangeInfo(buffer,start,start+obj.activityRecordLength-14,[],obj)
}
